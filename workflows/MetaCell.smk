rule metacell:
    ...
    output:
        mc_label="outputs/MetaCell/{sample}/K={K}/alpha={alpha}/metacell.csv",
        main=directory("outputs/MetaCell/{sample}/K={K}/alpha={alpha}"),
        mcdb=directory("outputs/MetaCell/{sample}/K={K}/alpha={alpha}/mcdb"),
        figs=directory("outputs/MetaCell/{sample}/K={K}/alpha={alpha}/figs")
    params:
        s="{sample}",
        K="{K}",
        alpha="{alpha}"
    log:
        "logs/MetaCell/{sample}/K={K}/alpha={alpha}/metacell.log"
    shell:
        """
...
rule mcrigor:
    ...
    output:
        out_dir=directory("outputs/mcRigor/{sample}"),
        mc_label="outputs/mcRigor/{sample}/metacell.csv",
        mc_res="outputs/mcRigor/{sample}/optimize_res.rds"
    params:
        sample_name="{sample}",
        Ks="30 40 50 60 70",
        alphas="0.6 0.8 1 1.2"
    log:
        "logs/mcRigor/{sample}.log"
    shell:
        """
        Rscript -e "rmarkdown::render(
